...
## MANDATORY: Template Update Check (Before Stage Transitions)

**Purpose**: Ensure project infrastructure stays current with the latest AI-DLC template improvements.

**When to Check**: Before updating Stage Progress in `aidlc-state.md` (i.e., before transitioning to a new stage).

**Execution**:
1. **Check if template remote exists**:
   ```bash
   git remote
   ```
   Look for `template` in the output. If not present, add it:
   ```bash
   git remote add template <TEMPLATE_URL>
   ```
   Replace `<TEMPLATE_URL>` with the template repository URL (e.g., `https://github.com/zrkidd-pixel/ai-dlc-template.git`).
...
3. **Check for infrastructure updates**:
   ```bash
   git diff HEAD template/main -- .aidlc-rule-details/ .kiro/ CLAUDE.md CHANGELOG.md
   ```

4. **If differences exist**:
   - Notify user: "Template updates available for AI-DLC infrastructure files."
   - List what changed (rule files, steering, CLAUDE.md, etc.)
   - Offer to pull updates:
     ```bash
     git checkout template/main -- .aidlc-rule-details/ .kiro/ CLAUDE.md CHANGELOG.md
     ```
   - **NEVER pull `aidlc-docs/`** — this contains project-specific work that must be preserved.
   - After pulling, commit the updates before proceeding with stage work.

5. **If no differences**: Proceed silently with the stage transition.

**Note**: This check is lightweight and only runs at stage boundaries, not during normal workflow execution. The first time this runs on a project created from the template, you'll need to add the template remote.
...
**Purpose**: Post-construction activities including UI/UX refinements, deployment, and maintenance

**Focus**: How to REFINE, DEPLOY, and RUN it
...
**Execute IF**:
- Application is functional (CONSTRUCTION complete or existing working app)
- User requests visual/design improvements or redesign
- Design references or inspiration are available or can be collected

**Skip IF**:
- New feature development needed (use CONSTRUCTION instead)
- No UI changes requested

**Execution**:
1. **MANDATORY**: Log any user input during this stage in audit.md
2. Load all steps from `operations/ui-ux-refinements.md`
3. Execute reference collection:
   - Request design screenshots/inspiration from user
   - Analyze reference materials for patterns
   - Extract color palette, typography, spacing, components
4. Generate UI/UX questions using [Answer]: tag format
5. Create design system document
6. Generate screen-by-screen implementation plan
7. Execute implementation (design system → components → screens)
8. **Wait for Explicit Approval**: Present completion message - DO NOT COMMIT/PUSH until user confirms
9. **MANDATORY**: Log user's response in audit.md with complete raw input

## Operations Placeholders

The following stages are placeholders for future expansion:
...
│   │   ├── plans/
│   │   └── ui-ux-refinements/      # Design system, reference analysis
│   ├── aidlc-state.md
│   └── audit.md
```
